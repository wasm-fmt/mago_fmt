...
printf(
    '<a href="%1$s" class="%2$s">%3$s</a>',
    esc_url(add_query_arg(array('tab' => $slug), admin_url('site-health.php'))),
    $current_tab === $slug ? 'active' : '',
    esc_html($label),
);

// Multi-line array in static method call
$wrapper = Html::rawElement(
    'div',
    array_merge(['class' => [
        'mw-toplinks-content',
...
$wrapper = Html::rawElement(
    'div',
    array_merge(['class' => ['mw-toplinks-content', 'mw-collapsible-content']], $langAttributes),
    $content,
);
