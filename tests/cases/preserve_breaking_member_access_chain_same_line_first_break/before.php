...
$queryBuilder->leftJoin(sprintf('%s.hosts', $rootAlias), 'announcement_host_filter')->andWhere($queryBuilder->expr()->orX('announcement_host_filter.id = :announcementHostId', 'announcement_host_filter.id IS NULL'))->setParameter('announcementHostId', $host->getId()?->toBinary());

$context->buildViolation('Please provide a flat price when selling early check-in.')
...
$validatorFactory->createForEntity($entity)->withScenario('night-audit')->withSeverity('warning')->markAsSkipped()
;

$validatorFactory->createForEntity($entity)->withScenario('night-audit');

Network\SocketOptions::create()->withAddressReuse(false)->withPortReuse(false)->withBroadcast(true)
;
