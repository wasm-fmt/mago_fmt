...
final class SchemaPropertyMetadataFactory implements PropertyMetadataFactoryInterface
{
    ...
    public function create(string $resourceClass, string $property, array $options = []): ApiProperty
    {
        ...
        $link = ($options['schema_type'] ?? null) === Schema::TYPE_INPUT
        ...
        if (
            ...
            || !\array_key_exists('readOnly', $propertySchema)
            && false === $propertyMetadata->isWritable()
        ...
        if (
            !\array_key_exists('description', $propertySchema)
            && null !== ($description = $propertyMetadata->getDescription())
        ) {
        ...
        if (
            !\array_key_exists('externalDocs', $propertySchema)
            && null !== ($iri = $propertyMetadata->getTypes()[0] ?? null)
        ) {
            $propertySchema['externalDocs'] = ['url' => $iri];
        }

        if (!method_exists(PropertyInfoExtractor::class, 'getType')) {
            return $propertyMetadata->withSchema($this->getLegacyTypeSchema(
                $propertyMetadata,
                $propertySchema,
                $resourceClass,
                $property,
    ...
    private function getTypeSchema(ApiProperty $propertyMetadata, array $propertySchema, ?bool $link): array
    {
        ...
        $typeIsResourceClass = function (Type $type) use (&$className): bool {
            ...
            && $this->resourceClassResolver->isResourceClass($className = $type->getClassName());
        };
        ...
        if (
            !\array_key_exists('example', $propertySchema)
            && null !== ($example = $propertyMetadata->getExample())
            && false === (\is_array($example) && empty($example))
        ) {
            $propertySchema['example'] = $example;
        }

        $hasType =
            $this->getSchemaValue($propertySchema, 'type')
            || $this->getSchemaValue($propertyMetadata->getJsonSchemaContext() ?? [], 'type')
            || $this->getSchemaValue($propertyMetadata->getOpenapiContext() ?? [], 'type');
        $hasRef =
            $this->getSchemaValue($propertySchema, '$ref')
            || $this->getSchemaValue($propertyMetadata->getJsonSchemaContext() ?? [], '$ref')
            || $this->getSchemaValue($propertyMetadata->getOpenapiContext() ?? [], '$ref');
    ...
    private function getJsonSchemaFromType(Type $type, ?bool $readableLink = null): array
    {
        ...
        if ($type instanceof UnionType) {
            $subTypes = array_filter(
                $type->getTypes(),
                static fn($t) => !($t instanceof BuiltinType && $t->isIdentifiedBy(TypeIdentifier::NULL)),
            );
        ...
        if ($type instanceof CollectionType) {
            ...
            if ($keyType->isSatisfiedBy(
                static fn(Type $t) => $t instanceof BuiltinType && $t->isIdentifiedBy(TypeIdentifier::INT),
            )) {
                $schema = [
                    'type' => 'array',
                    'items' => $valueSchema,
    ...
    private function getLegacyTypeSchema(
        ...
        if (
            !\array_key_exists('default', $propertySchema)
            && !empty($default = $propertyMetadata->getDefault())
            && (!$className || !$this->isResourceClass($className))
        ) {
            if ($default instanceof \BackedEnum) {
                $default = $default->value;
            }
        ...
        foreach ($types as $type) {
            ...
            $propertyType = $this->getLegacyType(
                new LegacyType($builtinType, $type->isNullable(), $className, $isCollection, $keyType, $valueType),
                $link,
            );
    ...
    private function legacyTypeToArray(LegacyType $type, ?bool $readableLink = null): array
    {
        return match ($type->getBuiltinType()) {
            ...
            LegacyType::BUILTIN_TYPE_OBJECT => $this->getLegacyClassType(
                $type->getClassName(),
                $type->isNullable(),
                $readableLink,
            ),
    ...
    private function getSchemaValue(array $schema, string $key): array|string|null
    {
        ...
        return (
            $schema[$key] ?? $schema['allOf'][0][$key] ?? $schema['anyOf'][0][$key] ?? $schema['oneOf'][0][$key] ?? null
        );
    }
}
