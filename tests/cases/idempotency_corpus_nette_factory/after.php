...
final class Factory
{
    ...
    public function fromClassReflection(\ReflectionClass $from, bool $withBodies = false): ClassLike
    {
    ...
    private function setupInheritance(ClassLike $class, \ReflectionClass $from): void
    {
        ...
        if ($from->getParentClass()) {
            ...
            $class->setImplements(array_values(array_diff(
                $class->getImplements(),
                $from->getParentClass()->getInterfaceNames(),
            )));
        }
    }
    ...
    private function populateMembers(ClassLike $class, \ReflectionClass $from, bool $withBodies): void
    {
        ...
        foreach ($from->getMethods() as $method) {
            ...
            if (
                $declaringClass->name === $from->name
                && (
                    !$from instanceof \ReflectionEnum
                    || !method_exists($from->isBacked() ? \BackedEnum::class : \UnitEnum::class, $method->name)
                )
            ) {
    ...
    public function fromFunctionReflection(\ReflectionFunction $from, bool $withBody = false): GlobalFunction|Closure
    {
        ...
        if ($withBody) {
            if ($from->isClosure() || $from->isInternal() || !($file = $from->getFileName())) {
                throw new Nette\NotSupportedException(
                    'The $withBody parameter cannot be used for closures or internal functions.',
                );
            }
    ...
    private function addHooks(\ReflectionProperty $from, Property|PromotedParameter $prop): void
    {
        ...
        $setV = $from->isPrivateSet() ? Visibility::Private : ($from->isProtectedSet() ? Visibility::Protected : $getV);
        $defaultSetV = $from->isReadOnly() && $getV !== Visibility::Private ? Visibility::Protected : $getV;
        if ($setV !== $defaultSetV) {
    ...
    private function getExtractor(string $file): Extractor
    {
        ...
        $cache ??= new Extractor(
            file_get_contents($file) ?: throw new Nette\InvalidStateException("Unable to read file '$file'."),
        );
        return $cache;
    }
}
