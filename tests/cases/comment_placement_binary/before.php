...
function check($a, $b, $c, $d) {
    return $a === $b && $c === $d // both pairs must match
        || $a === $c && $b === $d; // or cross-match
}
...
function long_chain($version_parts_count, $major_version, $version_parts, $last_part_split, $version_extra) {
    return $version_parts_count > 3
        || $version_parts_count < 2
        || !is_numeric($major_version)
        || ($version_parts_count === 3 && !is_numeric($version_parts[1]))
        || (!is_numeric($last_part_split[0]) && $last_part_split !== 'x' && $version_extra !== 'dev'); // validate version
}
