...
class CarbonPeriod extends DatePeriodBase implements Countable, JsonSerializable, UnitValue
{
    ...
    public static function instance(mixed $period): static
    {
        ...
        throw new NotAPeriodException(
            ...
            . end($chunks)
            . '() '
            . 'must be an instance of DatePeriod or '
            . $class
            . ', '
            . ($type === 'object' ? 'instance of ' . \get_class($period) : $type)
            . ' given.',
        );
    }
    ...
    public function __construct(...$arguments)
    {
        ...
        foreach ($arguments as $argument) {
            ...
            if ($argument instanceof DateTimeZone) {
                ...
                !isset($sortedArguments['interval']) && (
                    \is_string($argument) && preg_match(
                        '/^(-?\d(\d(?![\/-])|[^\d\/-]([\/-])?)*|P[T\d].*|(?:\h*\d+(?:\.\d+)?\h*[a-z]+)+)$/i',
                        $argument,
                    ) || $argument instanceof DateInterval || $argument instanceof Closure || $argument instanceof Unit
                ...
                !isset($sortedArguments['end']) && ($parsedDate = $parsedDate ?? $this->makeDateTime($argument))
            ) {
                $sortedArguments['end'] = $parsedDate;
                $originalArguments['end'] = $argument;
            } elseif (
                !isset($sortedArguments['recurrences'])
    ...
    public function setEndDate(mixed $date, ?bool $inclusive = null): static
    {
        if (
            $date !== null
            && !$this->isInfiniteDate($date)
            && !($date = [$this->dateClass, 'make']($date, $this->timezone))
        ) {
    ...
    private function getEndFromRecurrences(): ?CarbonInterface
    {
        ...
        if ($this->filters === [[static::RECURRENCES_FILTER, null]]) {
            return $this
                ->getStartDate()
                ->avoidMutation()
                ->add($this->getDateInterval()->times($this->carbonRecurrences - ($this->isStartExcluded() ? 0 : 1)));
        }
    ...
    {
        try {
            $values = array_combine(array_map(static fn(string $key): string => preg_replace(
                '/^\0\*\0/',
                '',
                $key,
            ...
            foreach ($values as $key => $value) {
                ...
                if ($value instanceof DateTimeInterface && !$value instanceof CarbonInterface) {
                    $value = $value instanceof DateTime ? Carbon::instance($value) : CarbonImmutable::instance($value);
                }
    ...
    private function initializeSerialization(array $values): void
    {
        ...
        parent::__construct(
            ...
            ($excludeStart ? self::EXCLUDE_START_DATE : 0)
            | ($includeEnd && \defined('DatePeriod::INCLUDE_END_DATE') ? self::INCLUDE_END_DATE : 0),
        );
